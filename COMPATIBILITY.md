...
### Regression checks
GitHub Actions runs `.github/workflows/ci.yml` on every push, pull request and manual dispatch. It checks PHP syntax and both PHP regression suites on PHP 8.0–8.5, WordPress coding standards/PHP compatibility rules on PHP 8.4, and JavaScript regression tests plus the asset build on Node 22. Dependencies are installed from the committed lockfiles. The build job fails if rebuilding changes tracked files under `assets/`; run `npm run build` and commit the generated assets alongside source changes.

These are regression checks, not a full WordPress/WooCommerce installation or live-browser end-to-end suite. The optional real WordPress HTML processor and FFmpeg checks below remain skipped in CI. To prevent merging failed checks, enable branch protection or a ruleset in GitHub and require all six `PHP tests (...)` checks, `PHP coding standards`, and `JavaScript tests and asset build` after the first workflow run.
